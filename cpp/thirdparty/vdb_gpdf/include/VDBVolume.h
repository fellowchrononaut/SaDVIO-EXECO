...
#pragma once
// SaDVIO: rclcpp removed; VDBVolume takes a VDBVolumeParams struct (see README.md in this folder)
...
// SaDVIO: the parameters the upstream constructor declared on its ROS node, with the same defaults
// (debug_print / use_color were declared by the upstream mapper node; both default to false here).
struct VDBVolumeParams {
    bool  debug_print = false;
    bool  use_color = false;
    float sdf_trunc = 0.1f;
    bool  space_carving = true;
    int   distance_method = 0;          // 0: reverting GPDF, 1: Log-GPDF
    int   sensor_type = 5;              // unused by the upstream algorithm (printed only)
    float voxel_size_local = 0.02f;
    int   voxel_overlapping = -1;
    int   voxel_downsample = 5;
    float voxel_size_global = 0.05f;
    int   variance_method = 0;
    float variance_cap = 0.1f;
    float variance_on_surface = 0.001f;
    int   surface_normal_method = 0;
...
public:
    explicit VDBVolume(const VDBVolumeParams& params); // SaDVIO: was VDBVolume(std::shared_ptr<rclcpp::Node>)
