...
// SaDVIO: parameters come from VDBVolumeParams instead of ROS node parameters
VDBVolume::VDBVolume(const VDBVolumeParams& p) {
  debug_print_ = p.debug_print;
  use_color_ = p.use_color;
  sdf_trunc_ = p.sdf_trunc;
  space_carving_ = p.space_carving;

  distance_method_ = p.distance_method;

  sensor_type_ = p.sensor_type;
  voxel_size_lo_ = p.voxel_size_local;
  voxel_overlapping_ = p.voxel_overlapping;
  voxel_downsample_ = p.voxel_downsample;
  voxel_size_gl_ = p.voxel_size_global;

  variance_method_ = p.variance_method;
  variance_cap_ = p.variance_cap;
  variance_on_surface_ = p.variance_on_surface;

  surface_normal_method_ = p.surface_normal_method;
  surface_normal_num_ = p.surface_normal_num;
  surface_value_ = p.surface_value;
  query_iterval_ = p.query_iterval;
  query_trunc_in_ = p.query_trunc_in;
  query_trunc_out_ = p.query_trunc_out;
  freespace_iterval_ = p.freespace_iterval;
  freespace_trunc_out_ = p.freespace_trunc_out;
  query_downsample_ = p.query_downsample;

  map_lambda_scale_ = p.map_lambda_scale;
  map_noise_ = p.map_noise;
  color_scale_ = p.color_scale;
  smooth_param_ = p.smooth_param;
...
void VDBVolume::GenerateVoxelsToUpdate(
  ...
  for (size_t Idx = 0; Idx < localGPsPoints_size; Idx = Idx+1) {
    ...
    if(surface_normal_method_ == 0){
      ...
      normalP.normalize();
      // SaDVIO: PCL leaves NaN normals for degenerate neighbourhoods; they turned into NaN query points and
      // aborted the KD-tree search. Fall back to the viewing direction (surface_normal_method 1) for those.
      if (!normalP.allFinite()) {
  ...
  LOG_IF(INFO, debug_print_ && nan_normals > 0) << "NaN PCL normals replaced by view direction: " << nan_normals << std::endl; // SaDVIO
